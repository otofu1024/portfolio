...
- Heroは参考画像の方向性に合わせ、大きな名前、背景文字、ドット装飾、プロフィールパネル背後の斜めグリーン面でポップさを出す。
- ヘッダーの下線は固定表示ではなく、`IntersectionObserver` によって現在表示中のセクションへ追従させる。
- `profileFacts` のアイコンは絵文字を使わず、一貫した線画SVGで表現する。
- 丸みのあるカードデザインは、Contactセクション内のメール・GitHubリンクに限定して使う。
- Skillsの使用技術は、言語を親、その関連ライブラリを子として見せる。現在は `Python` 配下に `FastAPI, httpx, asyncio, Pydantic` を表示し、`Rust` は単独で表示する。
